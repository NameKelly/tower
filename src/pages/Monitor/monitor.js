...
class Monitor extends React.Component {
              ...
              <Col span={13} style={{ marginBottom: 12 }} style={{ height: 180, marginBottom: 10 }}>
                <Card title='实时工作状态' style={{ height: "100%" }}
                  ...
                  <div style={{ display: 'flex', marginTop: 10, alignItems: 'center' }}>
                    ...
                    <div style={{ flex: 1, textAlign: 'center', fontSize: 18, color: 'green' }}><span>{realtimeData != {} ? realtimeData.sdt_x : ''}°</span></div>
                    <div style={{ flex: 1, textAlign: 'center', fontSize: 18, color: 'green' }}><span>{realtimeData != {} ? realtimeData.sdt_y : ''}°</span></div>
    ...
    request({
      ...
      success: ({
        ...
        if (store.sensorID_data[1]) {
          this.get_last(store.imei);//最后一条数据
  ...
  getDeviceData = (type, key, record) => {
    if (record) {
      store.imei = record.imei;
      this.get_last(record.imei);
    }
    let data = {};
    if (type == 'site_id') {
      data = {
    ...
    request({
      ...
      success: ({
        ...
        if (data.length > 0) {
          // store.realtimeData = data[0];//最近一条数据
